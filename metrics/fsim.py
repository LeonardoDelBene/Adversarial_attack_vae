...
class FSIM(Metric):
    ...
    def evaluate_folder(self, root_dir):
        ...
        for folder in tqdm(folders, desc="Evaluating FSIM", unit="folder"):
            ...
            orig_is_nan = np.isnan(fsim_orig)
            edited_is_nan = np.isnan(fsim_edited)

            if orig_is_nan:
                skipped_orig += 1
                print(f"[WARN] NaN FSIM (original vs immunized) in {folder}, excluded from average")
            else:
            ...
            if edited_is_nan:
                skipped_edited += 1
                print(f"[WARN] NaN FSIM (edited_original vs edited_immunized) in {folder}, excluded from average")
            else:
                fsim_edited_sum += fsim_edited
                valid_edited += 1

            result = {
                "fsim_original_vs_immunized": None if orig_is_nan else float(fsim_orig),
                "fsim_edited_original_vs_edited_immunized": None if edited_is_nan else float(fsim_edited),
            }
        ...
        avg_fsim_orig = fsim_orig_sum / valid_orig if valid_orig > 0 else 0.0
        avg_fsim_edited = fsim_edited_sum / valid_edited if valid_edited > 0 else 0.0
        ...
        with open(summary_path, "a", encoding="utf-8") as f:
            ...
            f.write(f"Samples excluded (NaN, original vs immunized): {skipped_orig}\n")
            f.write(f"Samples excluded (NaN, edited_original vs edited_immunized): {skipped_edited}\n")
            f.write(f"Average original vs immunized FSIM (valid={valid_orig}): {avg_fsim_orig:.4f}\n")
            f.write(f"Average edited_original vs edited_immunized FSIM (valid={valid_edited}): {avg_fsim_edited:.4f}\n")

        return {
            "total": total,
            "valid_original_vs_immunized": valid_orig,
            "valid_edited_original_vs_edited_immunized": valid_edited,
            "skipped_original_vs_immunized": skipped_orig,
            "skipped_edited_original_vs_edited_immunized": skipped_edited,
